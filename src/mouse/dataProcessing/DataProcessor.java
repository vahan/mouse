...
/**
 * Contains methods to read, process and store the data into appropriate db
 * tables
 * 
 * @author vahan
 * 
...
public class DataProcessor extends Observable implements Runnable {
	// TODO: The following constants can be changed to be read from the input
	// file and/or from an configuration file
	/**
	 * Constant representing the number of columns expected to be in the input
	 * CSV file
	 */
	...
	/**
	 * An array containing data transformed into "direction results" from
	 * "antenna readings"
	 */
	private final ArrayList<DirectionResultRow> directionResults = new ArrayList<DirectionResultRow>();
	/**
	 * An array containing data transformed into "stay results" from
	 * "direction results"
	 */
	private final ArrayList<StayResultRow> stayResults = new ArrayList<StayResultRow>();
	/**
	 * An array containing data transformed into "meeting results" from
	 * "stay results"
	 */
	...
	public void init(String inputCSVFileName, String boxDataFileName,
			Settings settings, boolean reset) {
	...
	private boolean process() {
		finished = false;
		if (!psqlManager.initTables(reset)) {
			return false;
		}
		if (!psqlManager.mergeStaticTables()) {
			return false;
		}
		...
		if (!storeAntennaReadings())
			return false;
		if (!storeExtremeResults(psqlManager.getAntennaReadings(),
				psqlManager.getAntennas(), new String[] { "last_reading" }, 0,
				0, true))
			return false;
		if (!storeExtremeResults(psqlManager.getAntennaReadings(),
				psqlManager.getTransponders(), new String[] { "last_reading",
						"last_antenna_id" }, 0, 1, true))
			return false;
		if (!storeExtremeResults(psqlManager.getAntennaReadings(),
				psqlManager.getTransponders(),
				new String[] { "first_reading" }, 0, 1, false))
			return false;
		generateDirAndStayResults();
		if (!storeDirectionResults())
			return false;
		if (!storeExtremeResults(psqlManager.getDirectionResults(),
				psqlManager.getBoxes(),
				new String[] { "last_direction_result" }, 0, 2, true))
			return false;
		if (!storeStayResults())
			return false;
		if (!generateMeetingResults())
			return false;
		if (!storeExtremeResults(psqlManager.getMeetingResults(),
				psqlManager.getBoxes(), new String[] { "last_meeting" }, 0, 2,
				true))
			return false;
	...
	private boolean readAntennaReadingsCSV(String sourceFile) {
		...
		CSVReader reader;
		try {
			reader = new CSVReader(new FileReader(sourceFile), ';', '\'', 1); // skip
																				// the
																				// first
																				// (header)
																				// line

			TimeStamp firstReading = null; // the earliest reading; for the Logs
											// table
			TimeStamp lastReading = null; // the latest reading; for the Logs
											// table
			...
			while ((nextLine = reader.readNext()) != null) {
				TimeStamp timeStamp;
				try {
					timeStamp = new TimeStamp(nextLine[DATE_TIME_STAMP_COLUMN],
							TimeStamp.getCsvForamt()); // read the timestamp
				...
				String boxName = nextLine[DEVICE_ID_COLUMN]; // read the box
																// name column
																// of the
																// current row
				BoxRow box = BoxRow.getBox(boxName); // get the box object via
														// its read name
				String antennaPosition = nextLine[ANTENNA_ID_COLUMN]; // read
																		// the
																		// antenna
																		// position
																		// (1 or
																		...
																		// current
																		// row
				AntennaRow antenna = AntennaRow.getAntennaRow(antennaPosition,
						box); // get the antenna object using its corresponding
								// box and position (id) on the box
				String rfid = nextLine[RFID_COLUMN]; // read the rfid column of
														// the current row
				if (StringUtils.isEmpty(rfid))
					continue; // TODO: skip the line if the rfid is missing?
				TransponderRow transponder = TransponderRow
						.getTransponderRow(rfid); // get the transponder object
													// via its rfid
				// create and the the antenna reading object to the
				// correcponding array
				AntennaReadingRow antennaReading = new AntennaReadingRow(
						timeStamp, transponder, antenna, log);
				antennaReadings.add(antennaReading);
				// store into mouseReadings array as well; needed for further
				// processing
				MouseRecords mouseRecords = mouseReadings.get(transponder);
				if (mouseRecords == null) {
					mouseRecords = new MouseRecords(transponder,
							new AntennaRecord(antenna, timeStamp));
	...
	private boolean storeAntennaReadings() {
		...
		antennaReadingsTable.setTableModels(antennaReadings
				.toArray(new AntennaReadingRow[antennaReadings.size()]));
		// Generate the INSERT query that inserts the data into the according db
		// table
		String insertQueries = antennaReadingsTable
				.insertQuery(antennaReadingsTable.getTableModels());
		// Execute the query and return the generated serial IDs
		String[] ids = psqlManager.executeQuery(insertQueries, "id");
		if (ids.length <= 0) {
	...
	private boolean storeLog() {
		...
		// Generate the INSERT query that inserts the data into the according db
		// table
		String insertQueries = logsTable
				.insertQuery(logsTable.getTableModels());
		// Execute the query and return the generated serial IDs
		String[] ids = psqlManager.executeQuery(insertQueries, "id");
		// Set the IDs to the appropriate objects
		if (ids.length != 1) {
	...
	/**
	 * A general method used to store extreme (earliest, latest) type of entries
	 * into various tables
	 * 
	 * @return true if successful, false - otherwise
	 */
	private boolean storeExtremeResults(DbDynamicTable dynamicTable,
			DbStaticTable staticTable, String[] fields, int extremeResultIndex,
			int staticTableRowIndex, boolean last) {
		notifyMessage("Updating " + staticTable.getTableName() + "."
				+ Arrays.toString(fields));
		dynamicTable.putExtremeReadings(extremeResultIndex,
				staticTableRowIndex, last);
		DbTableRow[] tableRows = staticTable.getTableModels();
		ColumnTypes[] types = new ColumnTypes[fields.length];
		for (int i = 0; i < fields.length; ++i) {
			types[i] = staticTable.getColumn(fields[i]).getType();
		}
		String updateLastReadingsQuery = staticTable.updateLastReadingsQuery(
				fields, tableRows, extremeResultIndex, types);
		String[] ids = psqlManager.executeQuery(updateLastReadingsQuery, "id");
	...
	private void generateDirAndStayResults() {
		for (Entry<TransponderRow, MouseRecords> entry : mouseReadings
				.entrySet()) {
			entry.getValue().addDirectionAndStayResults(directionResults,
					stayResults, settings.getMinTubeTime(),
					settings.getMaxTubeTime(), settings.getMinBoxTime(),
					settings.getMaxBoxTime());
		}
	}
	...
	private boolean storeDirectionResults() {
		/*
		 * //A dictionary to efficiently handle mouse-box - antenna-record time
		 * connections. HashMap<MouseInBox, AntennaRecord> mouseInBoxSet = new
		 * HashMap<MouseInBox, AntennaRecord>(); //Iterate through the antenna
		 * readings and transform the data into a direction results form for
		 * (AntennaReadingRow antennaReading : antennaReadings) { TransponderRow
		 * mouse = antennaReading.getTransponder(); AntennaRow antenna =
		 * (AntennaRow) antennaReading.getSource(); BoxRow box =
		 * antenna.getBox(); TimeStamp timestamp =
		 * antennaReading.getTimeStamp(); MouseInBox mouseInBox = new
		 * MouseInBox(mouse, box, antenna, timestamp); //represents an events of
		 * a mouse that appeared in a box AntennaRecord antennaRecord =
		 * mouseInBoxSet.get(mouseInBox); //get the exact antenna (and
		 * timestamp) that recorded the mouse showing up in the box //If the
		 * mouse-box pair is not already in the array, then add it; //meaning
		 * that the mouse was not recorded in this particular box before if
		 * (antennaRecord == null) { mouseInBoxSet.put(mouseInBox, new
		 * AntennaRecord(antenna, timestamp)); } else if
		 * (!antennaRecord.getAntenna().equals(antenna)) { //Otherwise if
		 * nothing 'illegal' happened add the direction result to the array
		 * AntennaRow in; AntennaRow out; if
		 * (timestamp.before(antennaRecord.getRecordTime())) { in = antenna; out
		 * = antennaRecord.getAntenna(); } else { in =
		 * antennaRecord.getAntenna(); out = antenna; } Direction direction =
		 * new Direction(in, out); if (direction.toString() == null) continue;
		 * TransponderRow transponder = antennaReading.getTransponder();
		 * DirectionResultRow dirResult = new DirectionResultRow(timestamp,
		 * direction, transponder, box); directionResults.add(dirResult);
		 * mouseInBoxSet.remove(mouseInBox); //Remove the mouse-in-box entry,
		 * when its pair is found } else { //If the mouse entered and never left
		 * the box before entering it again, //or left and never entered before
		 * living again, //count the time from the last recorded time
		...
		dirResultsTable.setTableModels(directionResults
				.toArray(new DirectionResultRow[directionResults.size()]));
		// Generate the INSERT query that inserts the data into the according db
		// table
		String insertQueries = dirResultsTable.insertQuery(dirResultsTable
				.getTableModels());
		// Execute the query and return the generated serial IDs
		String[] ids = psqlManager.executeQuery(insertQueries, "id");
		// Set the IDs to the appropriate objects
	...
	private boolean storeStayResults() {
		...
		stayResultsTable.setTableModels(stayResults
				.toArray(new StayResultRow[stayResults.size()]));
		// Generate the INSERT query that inserts the data into the according db
		// table
		String insertQueries = stayResultsTable.insertQuery(stayResultsTable
				.getTableModels());
		// Execute the query and return the generated serial IDs
		String[] ids = psqlManager.executeQuery(insertQueries, "id");
		// Set the IDs to the appropriate objects
	...
	private boolean generateMeetingResults() {
		...
		// A HashMap data structure to efficiently handle box-mouse-start-stop
		// connections.
		...
		for (Map.Entry<BoxRow, ArrayList<MouseInterval>> pair : boxSet
				.entrySet()) {
			BoxRow box = pair.getKey();
			// For each box iterate through the list of mouse-start-stop objects
			// checking if there are overlapping timestamps
			...
			for (MouseInterval mouseInterval : mouseIntervals) {
				...
				for (MouseInterval innerMouseInterval : mouseIntervals) {
					if (innerMouseInterval.getMouse() == mouseInterval
							.getMouse())
						continue;
					if (innerMouseInterval.getStart().before(
							mouseInterval.getStart()))
						continue; // Avoid duplicate entries
					TransponderRow transponderTo = innerMouseInterval
							.getMouse();
					TimeStamp start = mouseInterval.getStart().after(
							innerMouseInterval.getStart()) ? mouseInterval
							.getStart() : innerMouseInterval.getStart();
					TimeStamp stop = mouseInterval.getStop().before(
							innerMouseInterval.getStop()) ? mouseInterval
							.getStop() : innerMouseInterval.getStop();
					// Starting time must be before the stopping time to avoid
					// duplicate entries
					if (stop.before(start))
						continue;
					TransponderRow terminatedBy = mouseInterval.getStop()
							.before(innerMouseInterval.getStop()) ? transponderFrom
							: transponderTo;
					MeetingResultRow meetingResult = new MeetingResultRow(
							transponderFrom, transponderTo, start, stop,
							terminatedBy == transponderFrom ? 0 : 1, box);
					transponderFrom.addMeeting();
					transponderTo.addMeeting();
					if (!transponderFrom.getSex()
							.equals(transponderTo.getSex())) { // Unless they
																// are gay (or
																// maybe bi?)
		...
		meetingResultsTable.setTableModels(meetingResults
				.toArray(new MeetingResultRow[meetingResults.size()]));
		// Generate the INSERT query that inserts the data into the according db
		// table
		String insertQueries = meetingResultsTable
				.insertQuery(meetingResultsTable.getTableModels());
		// Execute the query and return the generated serial IDs
		String[] ids = psqlManager.executeQuery(insertQueries, "id");
		// Set the IDs to the appropriate objects
	...
	private boolean addTransponderCounts() {
		...
		String[] ids = psqlManager.executeQuery(psqlManager.getTransponders()
				.updateCountsQuery(), "id");
		if (ids.length > 0) {
	...
	/**
	 * Scans the input file and returns Columns of antennas, boxes and
	 * transponders. Needed for creating the static `antennas`, `boxes` and
	 * `transponders` tables. Must be called before the actual data processing
	 * starts
	 * 
	 * @param inputCSVFileName
	 *            The name of the input file
	 * @param unique
	 *            Determines if the generated columns contain only unique
	 *            entries
	 * @return Columns of antennas, boxes and transponders; null if reading
	 *         errors occur
	 */
	private CSVColumn[] csvColumns(String inputCSVFileName, boolean unique) {
		// Array of the column numbers
		int[] staticColumnNumbers = new int[] { ANTENNA_ID_COLUMN,
				DEVICE_ID_COLUMN, RFID_COLUMN };
		// Initializations
		...
		CSVReader reader;
		try {
			reader = new CSVReader(new FileReader(inputCSVFileName), ';', '\'',
	...
	private HashMap<String, BoxData> boxDataColumns(String inputFileName) {
		...
		CSVReader reader;
		try {
			reader = new CSVReader(new FileReader(inputFileName), '\t', '\'', 1); // skip
																					// the
																					// first
																					// (header)
																					// line
	...
	private boolean importBoxData(String fileName) {
		...
		DbTableRow[] boxRows = psqlManager.getBoxes().getTableModels();
		...
		String res[] = psqlManager.executeQuery(updateBoxDataQuery, "id");
		return res.length > 0;
