...
public class Boxes extends DbStaticTable {
	...
	public Boxes(String[] boxNames, boolean generate) {
		super("boxes", boxNames, generate);

		if (generate)
			generateTables();
		if (tableModels == null)
			tableModels = new BoxRow[names.length];
	}
	...
	protected void generateTables() {
		...
		for (int i = 0; i < names.length; ++i) {
			tableModels[i] = BoxRow.getBox(names[i]);
		}
	...
	protected String[] insertFields() {
		String[] fields = new String[] { "name", "segment", };
		return fields;
	}
	...
	public void setTableModels(Object[] array) {
		tableModels = new BoxRow[array.length];
	...
	protected void initColumns() {
		...
		columns.put("last_direction_result", new DbEntry(
				"last_direction_result", ColumnTypes.timestamp, ""));
		columns.put("last_meeting", new DbEntry("last_meeting",
	...
	private String updateQueryByName(String[] fields, String[][] values,
		...
		for (int i = 0; i < fields.length; ++i) {
			...
			for (int j = 0; j < tableModels.length; ++j) {
				...
				query += " WHEN " + quote + row.getName() + quote + " THEN "
						+ values[j][i];
			}
			query += " END";
		...
		query += "WHERE " + field + " IN (" + quote
				+ StringUtils.join(allIds, quote + ", " + quote) + quote + " )";
