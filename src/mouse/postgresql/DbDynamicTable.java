...
/**
 * Abstract base class modeling those db tables, that are to be filled after
...
public abstract class DbDynamicTable extends DbTable {
	 ...
	 * @return
	 */
	private HashMap<DbStaticTableRow, ExtremeReading> extremeReadings(
			int lastResultIndex, int staticTableRowIndex, boolean last) {
		...
		for (DbTableRow model : tableModels) {
			...
			DbStaticTableRow dbStaticTableRow = dynamicTableRow
					.staticTableRow(staticTableRowIndex);
			if (extremeReadings.containsKey(dbStaticTableRow)) {
				TimeStamp lastReading = extremeReadings.get(dbStaticTableRow)
						.getTimeStamp();
				if (last && timeStamp.after(lastReading) || !last
						&& timeStamp.before(lastReading)) {
					extremeReadings.put(dbStaticTableRow, new ExtremeReading(
							timeStamp, transponder, source));
				}
			} else {
				extremeReadings.put(dbStaticTableRow, new ExtremeReading(
						timeStamp, transponder, source));
	...
	public void putExtremeReadings(int extremeResultIndex,
		...
		for (DbStaticTableRow row : lastReadings.keySet()) {
			row.setLastResult(lastReadings.get(row).getTimeStamp(),
					extremeResultIndex);
			if (row instanceof TransponderRow) {
				DbStaticTableRow source = lastReadings.get(row).getSource();
				if (!(source instanceof AntennaRow))
					continue;
				lastReadings.get(row).getTransponder()
						.setLastAntennaId(((AntennaRow) source).getId());
			}
		}
	}
